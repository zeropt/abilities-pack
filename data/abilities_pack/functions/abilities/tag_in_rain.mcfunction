...
#in_rain
execute at @s unless predicate abilities_pack:dry_weather as @e[type=armor_stand,tag=rain_test] if score @s player_id = @p player_id store result score #abilities_pack apvar_0 run data get entity @s Fire
execute at @s unless predicate abilities_pack:dry_weather if score #abilities_pack apvar_0 matches 0 run tag @s add in_rain
